...
```bash
tock project list [-f <关键词>] [-o json]
tock project create <名称> [--color #F18181] [--view list|kanban|timeline] [--kind TASK|NOTE] [-o json]
tock project use <名称|id>          # 切换默认项目（持久化）
```

- `project list -f` 按名称子串过滤，结合 `-o json` 可精确定位 id。
- `project create` 返回含 `id` 的新清单对象；`--view` / `--kind` 可省略。
- `project use` 会改变持久默认值，优先用全局 `-P` 做一次性覆盖。
- 临时针对其他清单操作用全局 `-P`，**放在子命令之前**：
  `tock -P <projectId> task list -o json`。

用户说「购物清单」「工作任务」这类名字时，先 `project list -f <关键词> -o json`
找到 id，再用 `-P`。
